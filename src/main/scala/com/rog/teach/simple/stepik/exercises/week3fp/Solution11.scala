...
//Узел считается листом, если у него нет ни правого, ни левого потомка. Так что дерево, представленное на рисунке, имеет
// четыре листа со значениями 4, 8, 6, 7
//Примечание:
//  не забудьте дописать метод isLeaf
//  чтобы не перегружать рабочее пространство, код BinaryTree и TreeEnd был скрыт (он такой же, как код, представленный
//  выше). Простой учитывайте, что этот код есть и работать вам надо только с Node
//  принт результатов на вас (учитывайте,  что в коде прописан object Main extends App и вы можете сразу начинать писать
//  print)
...
//  как видно из определения функции, в результате вы получите List[BinaryTree[T]], т.е. для дерева, представленного в
//  качестве примера, результат будет List(Node(7,TreeEnd,TreeEnd), Node(6,TreeEnd,TreeEnd), Node(8,TreeEnd,TreeEnd),
//  Node(4,TreeEnd,TreeEnd)). Вам же надо вывести List(4, 6, 7, 8)
//  постарайтесь реализовать хвостовую рекурсию для метода collectLeaves

//Теперь поработаем с узлами дерева. Ваша задача на этом шаге - получить список значений узлов дерева на заданном
// уровне. Если посмотреть на иллюстрацию, то, например,  для уровня 2 вы должны в итоге получить список [4, 5, 6, 7]:
